...
def _validate_turn(state: GameState, actor: PlayerId, intent: TakeTurn) -> None:
    ...
    if intent.vote is not None:
        _validate_vote(state, actor, intent.vote)
...
def _validate_naming(state: GameState, intent: TakeTurn) -> None:
    ...
    for named in (intent.addressed, intent.accused):
        if named is not None:
            _ensure_alive_target(state, named)
...
def _validate_vote(state: GameState, actor: PlayerId, vote: Vote) -> None:
    if state.phase is not Phase.DAY:
